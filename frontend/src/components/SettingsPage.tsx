...
export const SettingsPage: React.FC = () => {
  ...
  const [profilePassword, setProfilePassword] = useState(() => {
    ...
    const flag = currentUser.role === 'Staff' ? 'hostel_registered_staff' : 'hostel_registered_heads';
    const list = JSON.parse(localStorage.getItem(flag) || '[]');
    const match = list.find((u: any) => u.email.toLowerCase() === currentUser.email.toLowerCase());
    return match ? match.password : '123456';
  });
  ...
  const handleSaveProfile = (e: React.FormEvent) => {
    ...
    updateUserProfile(profileName.trim(), profilePhone.trim(), profilePassword, profilePic);
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full rounded-lg border border-zinc-250 py-1.5 pr-8.5 pl-3 text-xs text-zinc-900 focus:border-zinc-800 focus:outline-hidden"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPass(!showPass)}
                    className="absolute top-2 right-2 flex items-center justify-center p-0.5 text-zinc-400 hover:text-zinc-600 transition"
                  >
